...
* Geocoding can take several hours, for this reason some code chunks in this script are not evaluated. Outputs yielded from the process are loaded from stored files to illustrate the results.
...
Queries to the Census Geocoding Service will be submitted through the user-friendly `tidygecoder` package (specifically we will use the `geocode` function)
...
for ( i in 1:loops ){
  ...
  res<- tibble( read.csv( paste0( "AddressNPO", i, ".csv" ) ) ) %>%
    geocode( ., street = Address, # specify key variables here appropriately
             city = City, 
             state = State,
             postalcode = Zip, 
             method = 'census',
